"""TEXT DOCUMENTS: plain text and Markdown files."""
...
def load_text(path: Path) -> Document:
    """A .txt file is one section; Chunking splits it."""
...
def load_markdown(path: Path) -> Document:
    ...
    sections = []
    for piece in splitter.split_text(_read(path)):
...
def _read(path: Path) -> str:
    # utf-8-sig drops a leading BOM (common in files saved by Windows editors).
